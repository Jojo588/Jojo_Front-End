    ...
    },
{
  ...
  return (
    <section className="min-h-fit mt-15 mb-20 px-6 py-12 md:px-20 bg-[#1A1A2E] text-[#EAEAEA] max-sm:mb-10">
      ...
      <div className="relative border-l-4 border-[#FCA311] pl-6 space-y-10">
        {experiences.map((exp, index) => (
          <div key={index} className="relative group">
            <div className="absolute -left-3 top-1.5 w-6 h-6 bg-[#FCA311] rounded-full border-4 border-[#1A1A2E] shadow-md"></div>
            <div className="ml-5">
              <h3 className="text-xl font-semibold">{exp.role}</h3>
              <span className="text-sm text-[#C5C5C5] italic">
                {exp.company} • {exp.date}
              </span>
              <p className="mt-2 text-[#C5C5C5] leading-relaxed">{exp.description}</p>
            </div>
          </div>
        ))}
      </div>
