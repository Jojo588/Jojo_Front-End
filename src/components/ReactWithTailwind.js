...
const ReactWithTailwind = ({ ReactwithtailwindList }) => {
  const scrollRef = useRef(null);
  const totalProjects = ReactwithtailwindList.length;
  const repeats = 5;
  const infiniteProjects = Array.from(
    { length: totalProjects * repeats },
    (_, i) => i % totalProjects
  );
...
export default ReactWithTailwind;
